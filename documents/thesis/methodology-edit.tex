...
\caption{Detection (blue) and discrimination (orange) rates are calculated after iteration of \textit{coneR1} (x-axis). Full results found in appendices.}
...
\section{Environmental Assessment - Environmental \hl{Properties}} \label{section:envassess}

\hl{We now create a proof-of-concept of our adaptive model, by correlating environmental properties with a previously assessed parameter \textit{coneR1}. We explore observed environmental properties within the datasets: attenuation and saturation.} \hl{We also perform sensitivity analysis on multiple environmental properties (brightness functions, SIFT parameters) to determine the extent of their possible influence on correlation between \textit{coneR1}* and} \hl{attenuation ($\alpha$).} \hl{A model is detailed for using the observed environmental properties to calculate a new variable \textit{coneR1}$'$, which represents the adaptation of the \textit{coneR1} parameter to more closely match the curated \textit{coneR1}* ideal value.}

\subsection{Previous Work - Large Region Texture Removal} \label{section:prevworkLRT}

\hl{Of all the leading shadow removal algorithms we evaluated, only one attempts to adapt its parameters to its current environment.} In Sanin et al.'s LRT shadow removal, there are three examples of ambient \hl{environment properties} taken into account: average saturation (of foreground pixels), average attenuation (from foreground to background), and average perimeter size of foreground objects. These three global properties govern the value of certain algorithm parameters.
...
This need for adaptability underscores the motivation of this research to discover a scalable and portable solution for parameterization. In the case of Physical shadow removal, foreground object perimeter has no bearing on the \hl{accuracy} of shadow removal, due to Physical removal utilizes a wholly pixel-based approach. However, both average saturation of foreground objects and average attenuation were further \hl{examined} in this study.

\subsection{Attenuation and Saturation}

\hl{After exploring previous work performed in section \ref{section:prevworkLRT}, we focus our research on evaluating the attenuation and saturation of an environment. In this section, we define attenuation and saturation.}
